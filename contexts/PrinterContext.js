...
export const PrinterProvider = ({ children }) => {
          ...
          try {
            ...
            if (btState === "PoweredOn") {
              ...
              try {
                ...
                // Fall through to other connection attempts
              }
              
              // Reduced delay for faster reconnection
              setTimeout(async () => {
                try {
                  // Try to get previously known devices
                  const knownDevices = await bleManager.devices([id]);
                  let deviceToConnect = null;
                  
                  if (knownDevices && knownDevices.length > 0) {
                    deviceToConnect = knownDevices[0];
                    console.log("Found previously connected device:", deviceToConnect.name || deviceToConnect.id);
                    await connectToPrinter(deviceToConnect);
                    setReconnectAttemptInProgress(false);
                  } else {
                    // If not found, scan for it with a shorter timeout
                    console.log("Device not immediately found, scanning for it...");
                    setIsScanning(true);
                    
                    // Listen for specific device only
                    bleManager.startDeviceScan([id], null, async (error, device) => {
                      if (error) {
                      ...
                      if (device && device.id === id) {
                        ...
                        console.log("Found device during scan:", device.id);
                    ...
                    // Stop scan after a shorter timeout for quicker feedback
                    setTimeout(() => {
                      bleManager.stopDeviceScan();
                      setIsScanning(false);
                      setReconnectAttemptInProgress(false);
                      console.log("Auto-reconnect scan timed out");
            ...
            console.error("Error during auto-reconnect:", stateError);
            setReconnectAttemptInProgress(false);
          }
        }
      } catch (error) {
      ...
      const monitorInterval = setInterval(async () => {
        try {
          ...
          try {
            ...
            // Often this is just a transient BLE stack issue
            return;
          }
          ...
          // This prevents false disconnections during navigation
        }
  ...
  }, [printerDevice, isConnected, autoReconnect, reconnectPrinter, reconnectAttemptInProgress, isPrinting]);
  ...
  const connectToPrinter = async (device) => {
    ...
    try {
      ...
      // This ensures the printer is ready after auto-reconnection
      try {
  ...
  const scanForPrinters = async () => {
    ...
    try {
      ...
      // Start scanning for ALL devices, matching OrderCreate.js approach
      try {
    ...
    try {
      ...
      try {
        const device = await bleManager.connectToDevice(lastConnectedId, {
          autoConnect: true,
          timeout: 8000  // Increased from 5000 to 8000 for more reliability
        });
        ...
        if (device) {
          ...
          try {
            ...
            // Continue even if service discovery fails
      ...
      if (!device) {
        console.log("Device not in known devices, scanning for it");
        setIsScanning(true);
        ...
        return new Promise((resolve) => {
          ...
          // Stop scan after shorter timeout
          setTimeout(() => {
            try {
              if (bleManager) {
                bleManager.stopDeviceScan();
  ...
  const sendDataToPrinter = async (data) => {
    ...
    try {
      ...
      const CHUNK_SIZE = 150; // Increased from 100 to 150 bytes per chunk
      for (let i = 0; i < data.length; i += CHUNK_SIZE) {
        ...
        try {
          ...
          // Reduced delay between chunks to speed up printing while still preventing buffer overflows
          if (i + CHUNK_SIZE < data.length) {
            await new Promise(resolve => setTimeout(resolve, 100)); // Decreased from 200ms to 100ms
          }
        } catch (writeError) {
