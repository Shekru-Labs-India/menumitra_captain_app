...
const WaiterStaff = () => {
    ...
    const fetchStaff = async () => {
        try {
            ...
            const response = await axios.post(onGetProductionUrl()+'/get_staff_list_with_role', {
    ...
    const handleEditPress = (item) => {
        navigation.navigate('EditStaff', { staffId: item.staff_id, refresh: fetchStaff });
    };
    ...
    return (
        <View style={styles.container}>
            <View style={globalStyles.searchContainer}>
                ...
                />
                <Icon name="search" size={20} color="#6200ee" style={globalStyles.searchIcon} />
            </View>
            <FlatList
                data={filteredPosts}
                renderItem={renderItem}
                keyExtractor={item => item.staff_id.toString()}
                contentContainerStyle={Platform.OS === 'web' ? styles.fullWidthList : {}}
            />
            <TouchableOpacity style={globalStyles.addButton} onPress={handleAddNewPress}>
                <RemixIcon name="ri-add-circle-line" size={24} color="#fff" />
                <Text style={globalStyles.addButtonText}>Add New Staff</Text>
            </TouchableOpacity>
        </View>
    );
};
